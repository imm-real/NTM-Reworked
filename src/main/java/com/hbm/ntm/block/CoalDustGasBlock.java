...
/** Source airborne coal dust emitted by mined coal and lignite blocks. */
public final class CoalDustGasBlock extends Block {
    ...
    protected void entityInside(BlockState state, Level level, BlockPos pos, Entity entity) {
        if (!level.isClientSide && entity instanceof LivingEntity living
                && !HazardSystem.hasProtection(living, HazardProtection.PARTICLE_COARSE, 0)) {
            RadiationSystem.addBlackLung(living, 10);
        }
    }
    ...
    private boolean move(ServerLevel level, BlockPos from, Direction direction) {
        ...
        if (!level.getBlockState(to).isAir()) {
            level.scheduleTick(from, this, MOVE_DELAY);
            return false;
        }
