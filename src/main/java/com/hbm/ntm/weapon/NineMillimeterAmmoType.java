...
/** Four 9mm loads with IDs 20 through 23. */
public enum NineMillimeterAmmoType implements SednaAmmoType {
    SOFT_POINT("p9_sp", 20, 22, 1.0F, false, true,
            1.25F, 0.0F, 0.0F),
    FULL_METAL_JACKET("p9_fmj", 21, 23, 0.8F, false, true,
            1.25F, 2.0F, 0.1F),
    HOLLOW_POINT("p9_jhp", 22, 24, 1.5F, false, true,
            1.5F, 0.0F, -0.25F),
    ARMOR_PIERCING("p9_ap", 23, 25, 1.25F, true, false,
            1.25F, 5.0F, 0.15F);
    ...
    NineMillimeterAmmoType(String serializedName, int legacyMetadata, int legacyBulletConfig,
    ...
    public static NineMillimeterAmmoType fromLegacyMetadata(int metadata) {
        for (NineMillimeterAmmoType type : values()) if (type.legacyMetadata == metadata) return type;
        return SOFT_POINT;
    }

    public static NineMillimeterAmmoType fromLegacyBulletConfig(int config) {
        for (NineMillimeterAmmoType type : values()) if (type.legacyBulletConfig == config) return type;
        return SOFT_POINT;
    }
}
