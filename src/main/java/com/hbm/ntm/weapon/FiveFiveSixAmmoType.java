...
/** Four 5.56 mm loads from the XFactory556mm cabinet. */
public enum FiveFiveSixAmmoType implements SednaAmmoType {
    SOFT_POINT("r556_sp", 24, 1.0F, false, true, 1.25F, 0.0F, 0.0F),
    FULL_METAL_JACKET("r556_fmj", 25, 0.8F, false, true, 1.25F, 4.0F, 0.1F),
    HOLLOW_POINT("r556_jhp", 26, 1.5F, false, true, 1.5F, 0.0F, -0.25F),
    ARMOR_PIERCING("r556_ap", 27, 1.25F, true, false, 1.25F, 10.0F, 0.15F);
    ...
    FiveFiveSixAmmoType(String serializedName, int legacyMetadata, float damageMultiplier,
    ...
    public static FiveFiveSixAmmoType fromLegacyMetadata(int metadata) {
        for (FiveFiveSixAmmoType type : values()) if (type.legacyMetadata == metadata) return type;
        return SOFT_POINT;
    }

    public static FiveFiveSixAmmoType fromLegacyBulletConfig(int config) {
