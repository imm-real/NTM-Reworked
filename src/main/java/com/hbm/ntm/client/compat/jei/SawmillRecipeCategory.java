...
public final class SawmillRecipeCategory implements IRecipeCategory<SawmillJeiRecipe> {
    ...
    public SawmillRecipeCategory(IGuiHelper gui) {
        background = gui.createDrawable(TEXTURE, 5, 11, 166, 65);
        icon = gui.createDrawableItemLike(ModItems.MACHINE_SAWMILL_ITEM.get());
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, SawmillJeiRecipe recipe,
                          IFocusGroup focuses) {
        ...
        if (recipe.sawdustChance() > 0) {
            builder.addOutputSlot(120, 24)
                    .setBackground(slot, -1, -1)
                    .addItemStack(new ItemStack(ModItems.POWDER_SAWDUST.get()))
                    .addRichTooltipCallback((view, tooltip) -> tooltip.add(
                            Component.literal(recipe.sawdustChance() + "%")
                                    .withStyle(ChatFormatting.RED)));
        }
        builder.addSlot(RecipeIngredientRole.RENDER_ONLY, 75, 31)
                .addItemStack(new ItemStack(ModItems.MACHINE_SAWMILL_ITEM.get()));
    }

    @Override
    public void draw(SawmillJeiRecipe recipe, IRecipeSlotsView slots, GuiGraphics graphics,
    ...
    @Override
    public ResourceLocation getRegistryName(SawmillJeiRecipe recipe) {
        return recipe.id();
    }
}
