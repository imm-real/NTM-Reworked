...
public final class HeavyRevolverItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public HeavyRevolverItemRenderer() {
    ...
    private static Animation animation(ItemStack stack) {
        float partial = Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true);
        double time = (HeavyRevolverItem.animationTimer(stack) + partial) * 50.0D;
        return switch (HeavyRevolverItem.animation(stack)) {
            ...
            case EQUIP -> new Animation(ZERO,
                    sequence(time, frame(90,0,0,0), frame(0,0,0,500,Curve.SIN_DOWN)),
