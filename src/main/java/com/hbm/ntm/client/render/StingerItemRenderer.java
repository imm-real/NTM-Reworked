...
/** Stinger mesh, sight, rocket, warning label and several military poses. */
public final class StingerItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation MODEL = id("models/weapons/stinger.obj");
    private static final ResourceLocation TEXTURE = id("textures/models/weapons/stinger.png");
    private static final Set<String> GROUPS = Set.of("Mag_Cube.002", "Cube.001_Cube.002");
    private static final ResourceLocation ROCKET_MODEL = id("models/weapons/panzerschreck.obj");
    private static final ResourceLocation ROCKET_TEXTURE =
            id("textures/models/weapons/panzerschreck.png");
    private static final Set<String> ROCKET_GROUP = Set.of("Rocket");

    private EnvsuitMesh mesh;
    private EnvsuitMesh rocketMesh;

    public StingerItemRenderer() {
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poses,
        ...
        if (!(stack.getItem() instanceof StingerLauncherItem)) return;
        ...
        // ItemRenderStinger deliberately hides the complete weapon at full sight alignment.
        if (firstPerson && ClientWeaponEvents.aimingProgress(partial) >= 1.0F) return;
        ...
        if (firstPerson) {
            ...
            renderStinger(poses, buffers, light, overlay);
    ...
    private void renderFirstPerson(ItemStack stack, PoseStack poses,
        ...
        poses.pushPose();
        poses.translate(rocket.x, rocket.y + 3.5D, rocket.z - 3.0D);
        rocketMesh().render("Rocket", poses.last(),
                buffers.getBuffer(RenderType.entityCutout(ROCKET_TEXTURE)),
                1.0F, light, overlay, -1);
        renderRocketLabel(poses, buffers);
        poses.popPose();
    }

    private static void renderRocketLabel(PoseStack poses, MultiBufferSource buffers) {
        String label = "Not accurate";
        Font font = Minecraft.getInstance().font;
        float scale = 0.04F;
        poses.pushPose();
        poses.translate(0.025D, -0.5D, font.width(label) * 0.5D * scale - 3.0D);
        poses.scale(scale, -scale, scale);
        poses.mulPose(Axis.YP.rotationDegrees(90.0F));
        poses.mulPose(Axis.XN.rotationDegrees(45.0F));
        font.drawInBatch(label, 0.0F, 0.0F, 0xFFFF0000, false,
                poses.last().pose(), buffers, Font.DisplayMode.NORMAL,
                0, LightTexture.FULL_BRIGHT);
        poses.popPose();
    }

    private void renderStinger(PoseStack poses, MultiBufferSource buffers, int light, int overlay) {
        var consumer = buffers.getBuffer(RenderType.entityCutout(TEXTURE));
        for (String group : GROUPS) {
            mesh().render(group, poses.last(), consumer, 1.0F, light, overlay, -1);
        }
    }
    ...
    private EnvsuitMesh rocketMesh() {
        if (rocketMesh == null) {
            rocketMesh = EnvsuitMesh.load(Minecraft.getInstance().getResourceManager(),
                    ROCKET_MODEL, ROCKET_GROUP, "Stinger Rocket");
        }
        return rocketMesh;
    }
    ...
    private static void setupContext(ItemDisplayContext context, PoseStack poses, float partial) {
        ...
        switch (context) {
            case GUI -> {
                ...
                poses.mulPose(Axis.YP.rotationDegrees(90.0F));
                poses.scale(1.0625F / 16.0F, 1.0625F / 16.0F, 1.0625F / 16.0F);
                poses.mulPose(Axis.XP.rotationDegrees(25.0F));
                poses.mulPose(Axis.YP.rotationDegrees(225.0F));
                poses.translate(0.25D, -2.5D, 0.0D);
            }
            case GROUND, FIXED -> {
            ...
            case FIRST_PERSON_LEFT_HAND, FIRST_PERSON_RIGHT_HAND -> {
                ...
                poses.translate(lerp(side * 3.0D, side * 2.1D, aim),
                        lerp(-7.2D, -6.5D, aim), lerp(-2.8D, -8.5D, aim));
            }
            default -> {
                poses.scale(0.125F, 0.125F, 0.125F);
                poses.mulPose(Axis.YN.rotationDegrees(90.0F));
    ...
    private static void setupThirdPerson(ItemDisplayContext context, PoseStack poses) {
        ...
        poses.scale(1.5F, 1.5F, 1.5F);
        poses.translate(0.0D, -2.5D, -3.5D);
        poses.mulPose(Axis.YP.rotationDegrees(180.0F));
    }

    private static double equipAngle(ItemStack stack) {
        if (StingerLauncherItem.animation(stack) != StingerLauncherItem.GunAnimation.EQUIP) {
            return 0.0D;
        }
    ...
    private static double reloadAngle(ItemStack stack) {
        StingerLauncherItem.GunAnimation animation = StingerLauncherItem.animation(stack);
        double time = animationTime(stack);
        if (animation == StingerLauncherItem.GunAnimation.RELOAD) {
            if (time < 750.0D) return 90.0D * sinFull(time / 750.0D);
            if (time < 1750.0D) return 90.0D;
            if (time < 2500.0D) {
                return 90.0D * (1.0D - sinFull((time - 1750.0D) / 750.0D));
            }
            return 0.0D;
        }
        if (animation == StingerLauncherItem.GunAnimation.INSPECT
                || animation == StingerLauncherItem.GunAnimation.JAMMED) {
            if (time < 750.0D) return 90.0D * sinFull(time / 750.0D);
            if (time < 1250.0D) return 90.0D;
            if (time < 2000.0D) {
                return 90.0D * (1.0D - sinFull((time - 1250.0D) / 750.0D));
            }
        }
        return 0.0D;
    }
    ...
    private static Vec rocketPosition(ItemStack stack) {
        ...
        if (animation == StingerLauncherItem.GunAnimation.RELOAD) {
            ...
            if (time < 1250.0D) {
                double amount = Math.sin(Mth.clamp((time - 750.0D) / 500.0D, 0.0D, 1.0D)
                        * Math.PI * 0.5D);
                return new Vec(0.0D, lerp(-3.0D, 0.0D, amount),
                        lerp(-6.0D, -6.5D, amount));
            }
            if (time < 1600.0D) {
                double amount = 1.0D - Math.cos(
                        Mth.clamp((time - 1250.0D) / 350.0D, 0.0D, 1.0D)
                                * Math.PI * 0.5D);
        ...
        if (animation == StingerLauncherItem.GunAnimation.INSPECT) {
            return new Vec(0.0D, StingerLauncherItem.rounds(stack) <= 0 ? -3.0D : 0.0D, 0.0D);
        }
        return Vec.ZERO;
    }
    ...
    private static void renderThirdFlash(PoseStack poses, MultiBufferSource buffers,
                                         float progress, float random) {
        poses.pushPose();
        poses.translate(0.0D, 3.5D, -10.3795D);
        poses.mulPose(Axis.YP.rotationDegrees(90.0F));
        poses.mulPose(Axis.XP.rotationDegrees(random * 90.0F));
        poses.scale(0.75F, 0.75F, 0.75F);
        SednaMuzzleFlash.render(poses, buffers, progress, 10.0F);
        poses.popPose();
    }

    private static double animationTime(ItemStack stack) {
        float partial = Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true);
        return (StingerLauncherItem.animationTimer(stack) + partial) * 50.0D;
    }
